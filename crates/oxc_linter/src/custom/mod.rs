...
use crate::{
    ...
    rules::{
        ...
        EslintNoUnexpectedMultiline, EslintNoUnneededTernary, EslintNoUnusedVars, RuleEnum,
    },
};
...
impl CustomLinter {
    ...
    fn get_eslint_rules(&self) -> Vec<RuleWithSeverity> {
        ...
        vec![
            ...
            // TODO: 循环内不检查
            c!(2, EslintNoConstantCondition, json!({"checkLoops":false})),
            ...
            c!(2, EslintNoDuplicateImports, json!({"includeExports":true})),
            c!(0, EslintNoElseReturn, json!({"allowElseIf":true})),
            ...
            c!(severity_by_mode, EslintNoEmpty, json!({"allowEmptyCatch":true})),
            c!(2, EslintNoEqNull),
            // 允许这种逃逸手段
            // const indirect = eval;
            // indirect('var x = 1');
            c!(severity_by_mode, EslintNoEval, json!({"allowIndirect":true})),
            c!(2, EslintNoExAssign),
            c!(2, EslintNoExtendNative, json!({"exceptions":[]})),
            c!(2, EslintNoExtraBooleanCast, json!({"enforceForLogicalOperands":true})),
            c!(2, EslintNoExtraLabel),
            c!(
                2,
                EslintNoFallthrough,
                json!({"allowEmptyCase":true, "reportUnusedFallthroughComment":true})
            ),
